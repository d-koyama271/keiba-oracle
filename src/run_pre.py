...
def run_pre_flow(config: dict, target_date: str | None, job_name: str = "pre", *, phase: str = "all") -> list[Path]:
    ...
    prediction_inputs = load_prediction_inputs(input_paths) if phase != "statistical" else {}
    statistical_inputs = {}
    if phase != "general":
        for path in paths:
            try:
...
def main() -> None:
    ...
    config = load_config()
    run_pre_flow(config, args.date, phase=args.phase)
